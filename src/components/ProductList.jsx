...
export default function ProductList({ products, onSelect, role, employees, onDelete }){
  const [showAll, setShowAll] = useState(false)
  const canDelete = onDelete && ['admin', 'manager'].includes(role)
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    {canDelete && (
                      <button
                        ...
                        className="inline-flex items-center justify-center rounded-lg px-3 py-2 bg-error/10 text-error font-semibold text-sm transition-all duration-300 hover:bg-error/20 hover:scale-105 active:scale-95 group"
                        aria-label="Supprimer le produit"
                        title="Supprimer ce produit"
                      >
                        <span className="material-symbols-outlined text-base group-hover:animate-pulse">delete_forever</span>
